...
@Component({
  selector: 'app-pass-bapu-bhawan',
  templateUrl: './pass-bapu-bhawan.component.html',
  styleUrls: ['./pass-bapu-bhawan.component.css']
})
export class PassBapuBhawanComponent implements OnInit {
  
  private bapuBhawanFormGroup: FormGroup = null;
  ...
  constructor(private route: ActivatedRoute, private passService: PassService) { 
    ...
    if(this.id == -1){
      ...
      this.passObject.passType = 5; //bapu bhawan 
  ...
  ngOnInit() {
    this.bapuBhawanFormGroup = new FormGroup ({
  ...
  private onSubmit(){
    ...
    console.log(this.bapuBhawanFormGroup)
    if(this.bapuBhawanFormGroup.valid){
     // code if the form is valid 
     console.log(this.bapuBhawanFormGroup)
    }
    else{
      // code in else , the form is invalid
      Object.keys(this.bapuBhawanFormGroup.controls).forEach(field =>{
        const control = this.bapuBhawanFormGroup.get(field);
